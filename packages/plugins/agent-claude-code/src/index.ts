...
async function setupHookInWorkspace(workspacePath: string): Promise<void> {
  ...
  if (isWindows()) {
    ...
    // Anchor to $CLAUDE_PROJECT_DIR (quoted for spaces) so the command resolves
    // regardless of the hook process cwd; a relative path breaks when the hook
    // fires with a cwd other than the worktree root.
